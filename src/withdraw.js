...
export default function Withdraw(){
    ...
    useEffect(() => {
        async function fetchdata() {
            await axios.get('https://server-zzo1.onrender.com/data').then((item) => {
             setData(item.data);
             console.log(data);
    ...
    async function submitHandler(e){    
        ...
        if(isNaN(withdraw) || withdraw < 0){
            ...
            await axios.put(`https://server-zzo1.onrender.com/update/${id}`,{amount:total-withdraw}).then( setTotal(total - withdraw)).then(alert(`amount ${withdraw} is withdrawn successfully !`));
        }
    }
